...
int removeMin(Heap* h, int i) {   // i는 힙 배열의 마지막 원소의 인덱스 (초기 i=5)
	Heap temp;
	int root = 1;
...
int IsIntheQ(int z, Heap* h, int idx) {   // 큐(힙 배열) 안에 정점 z가 있는지 확인  (초기 z=2, idx=5)
	int i;
...
void DijkstraShortestPaths() {   // Dijkstra 알고리즘
	int i, u, z;
	Heap* heap;
	Incidence* p;
	...
	while (i > 0) {     // 큐가 빌 때까지 반복
		...
		while (p != NULL) {    // 정점 u의 인접리스트를 처음부터 끝까지 훓는다 (u에 붙어있는 간선들을 모두 훓는다)
			...
			if (IsIntheQ(z, heap, i)) {    // 정점 z가 큐에 들어있다면
				if (G.Vert[u].dist + G.Edg[p->edge].weight < G.Vert[z].dist) {   // 다른 정점을 통해 z에 도착하는것 보다 u를 통해 z에 도착하는 것이 거리가 더 짧으면
					G.Vert[z].dist = G.Vert[u].dist + G.Edg[p->edge].weight;   // d(z)값 갱신

					replaceKey(z, G.Vert[u].dist + G.Edg[p->edge].weight, heap, i);    // 업데이트 된 z의 dist를 힙 배열에도 반영
				}
			}
			p = p->next;
		}
...
void upHeap(Heap* h, int idx) {   // 힙순서 속성 복구   (초기 idx=2)
